"""
Working query parser that correctly handles all query patterns
"""
import re
...
class WorkingQueryParser:
    """Working parser that correctly handles all query patterns"""
    ...
    def __init__(self):
        ...
        self.compositional_patterns = [
            ...
            # "does X increase/decrease Y" - must have clear X and Y (FIXED PATTERN)
            r'does\s+([^,\s]+(?:\s+[^,\s]+)*?)\s+(?:increase|decrease|reduce|improve|impair)\s+(?:the\s+)?(?:risk\s+of\s+)?(.+?)(?:\s*$|\s*\?)',
...
# Global instance
working_query_parser = WorkingQueryParser()
