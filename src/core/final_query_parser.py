"""
Final query parser that correctly handles both single-entity and compositional queries
"""
import re
...
class FinalQueryParser:
    """Final parser that correctly distinguishes single-entity vs compositional queries"""
    ...
    def __init__(self):
        ...
        # Relation cues for bonus scoring
        self.relation_cues = [
            "effect", "affect", "influence", "impact", "cause", "lead to", 
            "result in", "associated with", "correlated with", "linked to",
            "increases", "decreases", "reduces", "enhances", "inhibits",
            "promotes", "contributes to", "plays a role", "mechanism",
            "pathway", "process", "interaction", "relationship"
        ]
        
        # Clear compositional query patterns
        ...
        self.single_entity_indicators = [
            ...
            r'what\s+is\s+the\s+effect\s+of\s+',  # "what is the effect of X" - single entity
            r'how\s+does\s+',  # "how does X" - single entity
            r'what\s+are\s+the\s+effects\s+of\s+',  # "what are the effects of X" - single entity
            r'what\s+is\s+the\s+relationship\s+between\s+',  # "what is the relationship between X and Y" - could be compositional
        ]
    
    def extract_entities(self, query: str) -> ExtractedEntities:
        """Extract X and Y entities with improved single-entity detection"""
        query_lower = query.lower().strip()
        
        # First check if this looks like a single-entity query
    ...
    def _is_single_entity_query(self, query_lower: str) -> bool:
        ...
        # Special case: "what is the relationship between X and Y" should be compositional
        if 'what is the relationship between' in query_lower and ' and ' in query_lower:
            return False
    ...
    def _are_distinct_entities(self, x_raw: str, y_raw: str) -> bool:
        ...
        # If Y is just a single word or very short, it might not be a distinct entity
...
# Global instance
final_query_parser = FinalQueryParser()
