"""
Final query parser v2 that correctly handles all query patterns
"""
import re
...
class FinalQueryParserV2:
    """Final parser v2 that correctly handles all query patterns"""
    ...
    def __init__(self):
        ...
        self.synonyms = {
            ...
            'coffee': ['coffee', 'caffeine', 'coffee intake', 'coffee consumption'],
            'atrial_fibrillation': ['atrial fibrillation', 'afib', 'af', 'cardiac arrhythmia', 'heart rhythm']
        }
        ...
        self.compositional_patterns = [
            ...
            # "does X increase/decrease Y" - must have clear X and Y (improved pattern)
            r'does\s+([^,\s]+(?:\s+[^,\s]+)*?)\s+(?:increase|decrease|reduce|improve|impair)\s+(?:the\s+)?(?:risk\s+of\s+)?([^,\s]+(?:\s+[^,\s]+)*?)(?:\s|$|\?)',
    ...
    def extract_entities(self, query: str) -> ExtractedEntities:
        """Extract X and Y entities with improved pattern matching"""
        ...
        for pattern in self.compositional_patterns:
            ...
            if match:
                ...
                # Clean up Y term - remove trailing punctuation
                y_raw = re.sub(r'[.,!?;:]$', '', y_raw).strip()
...
# Global instance
final_query_parser_v2 = FinalQueryParserV2()
